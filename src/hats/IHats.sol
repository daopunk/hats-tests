...
interface IHatsIdUtilities {
    function buildHatId(uint256 _admin, uint8 _newHat) external pure returns (uint256 id);
    ...
    function getAdminAtLevel(uint256 _hatId, uint8 _level) external pure returns (uint256);
